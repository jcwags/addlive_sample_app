...
(function ($) {
  ...
  var APP_AUTH_SECRET = 'CloudeoTestAccountSecret';
  /**
   *
    ...
    initListener.onInitStateChanged = function (e) {
      switch (e.state) {
        ...
        case CDO.InitState.BROWSER_RESTART_REQUIRED:
          ...
          break;
        case CDO.InitState.UPDATE_BEGIN:
          log_d("Update process started");
          break;
        case CDO.InitState.DEVICES_INIT_BEGIN:
          log_d("Devices initialization begun");
          break;
        default:
...
//  CDO.initPlatform(initListener, CLIENT_ID);
  ...
  function initUI() {
    ...
    $('#camSelect').change(
        function () {
          ...
          CDO.getService().setVideoCaptureDevice(
              CDO.createResponder(function () {
    ...
    $('#micSelect').change(
        function () {
          var selected = $(this).val();
          CDO.getService().setAudioCaptureDevice(
              CDO.createResponder(function () {
    ...
    $('#spkSelect').change(
        function () {
          var selected = $(this).val();
          CDO.getService().setAudioOutputDevice(
              CDO.createResponder(function () {
                $('#micSelect').val(selected);
              }), selected);
        }
  ...
  function startPlugin() {
    ...
    CDO.getService().setApplicationId(CDO.createResponder(), APPLICATION_ID);
...
//  Initialize cameras
  ...
  function connect() {
    ...
    fillAuthDetails(connDescr, userId, scopeId);
    var succHandler = function (mediaConn) {
  ...
  function fillAuthDetails(connDescr, userId, scopeId) {
    // Old, fake authentication
    connDescr.token = userId + '';
    ...
    connDescr.authDetails = authDetails;
